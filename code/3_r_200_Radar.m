...
r=80;
%绘制4艘护卫舰与主舰的导弹覆盖范围
G6 =my_polar([0,20*pi/180],[0,200],'r-');%画直线
set( G6, 'LineWidth', 3 );
hold on;
z=200*cos(t)+i*(200*sin(t));%坐标为1,3，半径为2,4
G1=my_polar(angle(z),abs(z),'g-');
set( G1, 'LineWidth', 3 );
hold on;
z=R*cos(45*pi/180)+r*cos(t)+i*(R*cos(45*pi/180)+r*sin(t));%坐标为1,3，半径为2,4
G2=my_polar(angle(z),abs(z),'m--');
set( G2, 'LineWidth', 3 );
hold on;
...
G7 =my_polar([220*pi/180,220*pi/180],[5,200],'r-');%画直线
set( G7, 'LineWidth', 3 );
hold on;
...
legend( '导弹可能来袭扇面','预警机信息支援范围','舰队导弹覆盖范围','护卫舰位置','指挥舰位置');
b = findobj(gcf);
